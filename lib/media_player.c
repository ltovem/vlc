                         ...
                         void *data)
{
    ...
    mp->cbs->on_media_changed(mp->cbs_opaque, mp->p_md);
}
...
on_length_changed(vlc_player_t *player, vlc_tick_t new_length, void *data)
{
    ...
    mp->cbs->on_length_changed(mp->cbs_opaque, MS_FROM_VLC_TICK(new_length));
}
                                  ...
                                  input_item_t *media, void *data)
{
    ...
    if (atomic_compare_exchange_strong(&mp->p_md->parsed_status, &expected,
                                       libvlc_media_parsed_status_done)
     && mp->cbs != NULL && mp->cbs->on_media_parsed != NULL)
        mp->cbs->on_media_parsed(mp->cbs_opaque, mp->p_md);

    if (mp->cbs != NULL && mp->cbs->on_media_meta_changed != NULL)
        mp->cbs->on_media_meta_changed(mp->cbs_opaque, mp->p_md);
}
                          ...
                          input_item_node_t *new_subitems, void *data)
{
    ...
    if (media == current)
    {
        ...
        if (mp->cbs != NULL && mp->cbs->on_media_subitems_changed != NULL)
            mp->cbs->on_media_subitems_changed(mp->cbs_opaque, mp->p_md);
    }
}
