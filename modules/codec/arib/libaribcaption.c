...
{
    atomic_fetch_add_explicit(&p_sys->refs, (uintptr_t)1, memory_order_relaxed);
}

static void DecSysRelease(decoder_sys_t *p_sys)
{
    uintptr_t prev = atomic_fetch_sub_explicit(&p_sys->refs, (uintptr_t)1, memory_order_acq_rel);
    if (prev != 1)
        return;
                              ...
                              mtime_t i_ts)
{
    ...
    subpicture_updater_sys_t *p_spusys = p_subpic->updater.p_sys;
...
static int Decode(decoder_t *p_dec, block_t *p_block)
{
    ...
    subpicture_updater_sys_t *p_spusys = malloc(sizeof(subpicture_updater_sys_t));
    if (!p_spusys) {
        block_Release(p_block);
        return VLCDEC_SUCCESS;
    }
    ...
    if (caption.wait_duration == ARIBCC_DURATION_INDEFINITE) {
        ...
        p_spu->i_stop = p_block->i_pts + caption.wait_duration * 1000;
    }
...
static int Open(vlc_object_t *p_this)
{
    ...
    atomic_init(&p_sys->refs, (uintptr_t)1);
...
vlc_module_begin ()
    ...
    add_integer(ARIBCAPTION_CFG_PREFIX "rendering-backend", 0, CFG_TEXT_RENDERING_BACKEND, CFG_LONGTEXT_RENDERING_BACKEND, true)
        change_integer_list(rendering_backend_values, ppsz_rendering_backend_descriptions)
    add_font(ARIBCAPTION_CFG_PREFIX "font", DEFAULT_FAMILY, CFG_TEXT_FONT, CFG_LONGTEXT_FONT, true)
    add_bool(ARIBCAPTION_CFG_PREFIX "replace-drcs", true, CFG_TEXT_REPLACE_DRCS, CFG_LONGTEXT_REPLACE_DRCS, true)
    add_bool(ARIBCAPTION_CFG_PREFIX "force-stroke-text", false, CFG_TEXT_FORCE_STROKE_TEXT, CFG_LONGTEXT_FORCE_STROKE_TEXT, true)
    add_bool(ARIBCAPTION_CFG_PREFIX "ignore-background", false, CFG_TEXT_IGNORE_BACKGROUND, CFG_LONGTEXT_IGNORE_BACKGROUND, true)
    add_bool(ARIBCAPTION_CFG_PREFIX "ignore-ruby", false, CFG_TEXT_IGNORE_RUBY, CFG_LONGTEXT_IGNORE_RUBY, true)
    add_bool(ARIBCAPTION_CFG_PREFIX "fadeout", false, CFG_TEXT_FADEOUT, CFG_LONGTEXT_FADEOUT, true)
    add_float_with_range(ARIBCAPTION_CFG_PREFIX "stroke-width", 1.5f, 0.0f, 3.0f, CFG_TEXT_STROKE_WIDTH, CFG_LONGTEXT_STROKE_WIDTH, true)
vlc_module_end ()
