...
#define S (*p_synchro)
    ...
    case I_CODING_TYPE:
        if( b_low_delay )
        {
            pts = decoder_GetDisplayDate( p_synchro->p_dec, 0, now, S.current_pts );
        }
        else if( S.backward_pts != VLC_TICK_INVALID )
        {
            pts = decoder_GetDisplayDate( p_synchro->p_dec, 0, now, S.backward_pts );
        }
        else
        {
             ...
             *                      +- current PTS
             */
            pts = decoder_GetDisplayDate( p_synchro->p_dec, 0, now, S.current_pts ) + period * (S.i_n_b + 2);
        }
    ...
    case P_CODING_TYPE:
        ...
        {
            pts = decoder_GetDisplayDate( p_synchro->p_dec, 0, now, S.current_pts + period * (S.i_n_b + 1) );
        }
