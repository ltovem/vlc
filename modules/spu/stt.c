/*****************************************************************************
 * stt.c : spu module to use speech_to_text (stt) audio filter.
 *****************************************************************************
...
/*****************************************************************************
 * filter_sys_t: STT filter descriptor
 *****************************************************************************/
typedef struct
{
    vlc_mutex_t lock;
    ...
    char *format; /**< STT text format */
    char *message; /**< STT plain text */
...
#define TIMEOUT_LONGTEXT N_("Number of milliseconds the STT must remain " \
...
#define POS_TEXT N_("STT position")
#define POS_LONGTEXT N_( \
  "You can enforce the STT position on the video " \
...
#define CFG_PREFIX "stt-"

#define STT_HELP N_("Display text above the video")

/*****************************************************************************
 * Module descriptor
 *****************************************************************************/
vlc_module_begin ()
    set_shortname( N_("STT" ))
    set_description( N_("STT display") )
    set_help(STT_HELP)
...
static const char *const ppsz_filter_options[] = {
    "STT", "x", "y", "position", "color", "size", "timeout", "refresh",
    "opacity",
    NULL
};
...
/*****************************************************************************
 * CreateFilter: allocates STT video filter
 *****************************************************************************/
...
#define CREATE_VAR( stor, type, var ) \
    ...
    var_AddCallback( p_filter, var, STTCallback, p_sys );

    CREATE_VAR( i_xoff, Integer, "stt-x" );
    CREATE_VAR( i_yoff, Integer, "stt-y" );
    p_sys->i_timeout = VLC_TICK_FROM_MS(var_CreateGetIntegerCommand( p_filter,
                                                              "stt-timeout" ));
    var_AddCallback( p_filter, "stt-timeout", STTCallback, p_sys );
    p_sys->i_refresh = VLC_TICK_FROM_MS(var_CreateGetIntegerCommand( p_filter,
                                                              "stt-refresh" ));
    var_AddCallback( p_filter, "stt-refresh", STTCallback, p_sys );
    CREATE_VAR( i_pos, Integer, "stt-position" );
    CREATE_VAR( format, String, "stt-STT" );
    p_sys->message = NULL;
    p_sys->p_style->i_font_alpha = var_CreateGetIntegerCommand( p_filter,
                                                            "stt-opacity" );
    var_AddCallback( p_filter, "stt-opacity", STTCallback, p_sys );
    p_sys->p_style->i_features |= STYLE_HAS_FONT_ALPHA;
    CREATE_VAR( p_style->i_font_color, Integer, "stt-color" );
    p_sys->p_style->i_features |= STYLE_HAS_FONT_COLOR;
    CREATE_VAR( p_style->i_font_size, Integer, "stt-size" );
...
/*****************************************************************************
 * DestroyFilter: destroy STT video filter
 *****************************************************************************/
...
#define DEL_VAR(var) \
    var_DelCallback( p_filter, var, STTCallback, p_sys ); \
    var_Destroy( p_filter, var );
    DEL_VAR( "stt-x" );
    DEL_VAR( "stt-y" );
    DEL_VAR( "stt-timeout" );
    DEL_VAR( "stt-refresh" );
    DEL_VAR( "stt-position" );
    DEL_VAR( "stt-STT" );
    DEL_VAR( "stt-opacity" );
    DEL_VAR( "stt-color" );
    DEL_VAR( "stt-size" );
                            ...
                            void *p_data )
{
    ...
    vlc_mutex_lock( &p_sys->lock );
    if( !strcmp( psz_var, "stt-STT" ) )
    {
        free( p_sys->format );
        p_sys->format = strdup( newval.psz_string );
    }
    else if ( !strcmp( psz_var, "stt-x" ) )
    {
        p_sys->i_xoff = newval.i_int;
    }
    else if ( !strcmp( psz_var, "stt-y" ) )
    {
        p_sys->i_yoff = newval.i_int;
    }
    else if ( !strcmp( psz_var, "stt-color" ) )
    {
        p_sys->p_style->i_font_color = newval.i_int;
    }
    else if ( !strcmp( psz_var, "stt-opacity" ) )
    {
        p_sys->p_style->i_font_alpha = newval.i_int;
    }
    else if ( !strcmp( psz_var, "stt-size" ) )
    {
        p_sys->p_style->i_font_size = newval.i_int;
    }
    else if ( !strcmp( psz_var, "stt-timeout" ) )
    {
        p_sys->i_timeout = VLC_TICK_FROM_MS(newval.i_int);
    }
    else if ( !strcmp( psz_var, "stt-refresh" ) )
    {
        p_sys->i_refresh = VLC_TICK_FROM_MS(newval.i_int);
    }
    else if ( !strcmp( psz_var, "stt-position" ) )
    /* willing to accept a match against stt-pos */
    {
