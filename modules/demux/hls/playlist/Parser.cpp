...
HLSRepresentation * M3U8Parser::createRepresentation(BaseAdaptationSet *adaptSet, const AttributesTag * tag)
{
    ...
    try {
        rep = new HLSRepresentation(adaptSet);
    } catch(std::bad_alloc &) {
        return nullptr;
    }
    ...
    for(it = tagslist.begin(); it != tagslist.end(); ++it)
    {
        ...
        switch(tag->getType())
        {
            ...
            case SingleValueTag::URI:
            {
                ...
                try
                {
                    segment = new HLSSegment(rep, sequenceNumber++);
                } catch(std::bad_alloc &) {
                    break;
                }
...
M3U8 * M3U8Parser::parse(vlc_object_t *p_object, stream_t *p_stream, const std::string &playlisturl)
{
    ...
    if(b_masterplaylist)
    {
        ...
        for(groupsit = groupsmap.begin(); groupsit != groupsmap.end(); ++groupsit)
        {
            ...
            try {
                altAdaptSet = new BaseAdaptationSet(period);
            } catch(std::bad_alloc &) {
                altAdaptSet = nullptr;
            }
            if(altAdaptSet)
    ...
    {
        BaseAdaptationSet *adaptSet = nullptr;
        AttributesTag *tag = nullptr;
        try
        {
            adaptSet = new BaseAdaptationSet(period);
            tag = new AttributesTag(AttributesTag::EXTXSTREAMINF, "");
    ...
    while((psz_line = vlc_stream_ReadLine(stream)))
    {
        ...
        else if(*psz_line)
        {
            ...
            if(lastTag && lastTag->getType() == AttributesTag::EXTXSTREAMINF)
            {
                ...
                try
                {
                    Attribute *uriAttr = new Attribute("URI", std::string(psz_line));
