...
static void WindowMouseDoubleClick(void *opaque, libvlc_video_output_mouse_button_t button)
{
    vlc_window_t *window = opaque;
    vlc_window_ReportMouseDoubleClick(window, button);
}
...
static int Enable(struct vlc_window *wnd, const vlc_window_cfg_t *wcfg)
{
    ...
    if ( sys->setWindowCb != NULL )
        ...
        sys->setWindowCb( sys->opaque,
            WindowResize,
            WindowMouseMoved, WindowMousePress, WindowMouseRelease, WindowMouseDoubleClick,
            wnd );
...
static void Disable(struct vlc_window *wnd)
{
    ...
    if ( sys->setWindowCb != NULL )
        sys->setWindowCb( sys->opaque, NULL, NULL, NULL, NULL, NULL, NULL );
}
