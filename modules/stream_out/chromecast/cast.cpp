...
vlc_module_begin ()
    ...
    add_integer_with_range( SOUT_CFG_PREFIX "spu-text-opacity", 255, 0, 255,
        TEXT_OPACITY_TEXT, OPACITY_LONGTEXT )
        change_safe()
    ...
    add_integer_with_range( SOUT_CFG_PREFIX "spu-bg-opacity", 0, 0, 255,
        BG_OPACITY_TEXT, OPACITY_LONGTEXT )
        change_safe()
                                          ...
                                          const std::string &sout)
{
    ...
    p_spu_out = sout_StreamChainNew( VLC_OBJECT(p_stream), sout.c_str(), NULL);
