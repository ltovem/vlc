...
rav1e-vendor-build: .cargo .sum-rav1e
	...
	cd $@ && $(CARGO) vendor --locked
	cd $@ && tar -jcf rav1e-$(RAV1E_VERSION)-vendor.tar.bz2 vendor
...
rav1e-vendor: .sum-rav1e-vendor
	-$(RM) -R $@
	mkdir -p $@
	tar xjf $(TARBALLS)/rav1e-$(RAV1E_VERSION)-vendor.tar.bz2 -C $@
