            ...
            if (decoded) {
                if (is_late_dropped && !decoded->b_force)
                {
                    ...
                    const vlc_tick_t system_pts =
                        vlc_clock_ConvertToSystem(sys->clock, VLC_CLOCK_ID_LAST,
                            ...
                            vlc_render_subpicture **out_subpic)
{
    ...
    else
    {
        ...
        render_subtitle_date = filtered->date <= VLC_TICK_0 ? system_now :
            vlc_clock_ConvertToSystem(sys->clock, VLC_CLOCK_ID_LAST, system_now,
                                      filtered->date, sys->rate);
        vlc_clock_Unlock(sys->clock);
    }
...
static int RenderPicture(vout_thread_sys_t *sys, bool render_now)
{
    ...
    vlc_clock_Lock(sys->clock);
    vlc_tick_t drift = vlc_clock_UpdateVideo(sys->clock, VLC_CLOCK_ID_LAST,
                                             vlc_tick_now(),
                                             pts, sys->rate,
