                        ...
                        const vlc_subpicture_updater_params_t *params )
{
    ...
    if(!video_format_IsSimilar( cpy.p_fmt_src, &p_private->src ))
        cpy.flags |= VLC_SPU_UPDATER_FLAG_SOURCE_CHANGED;
    if(!video_format_IsSimilar( cpy.p_fmt_dst, &p_private->dst ))
        cpy.flags |= VLC_SPU_UPDATER_FLAG_DEST_CHANGED;
    if( !p_upd->pf_validate( p_subpicture, &cpy ) )
        return;

    subpicture_region_ChainDelete( p_subpicture->p_region );
    p_subpicture->p_region = NULL;

    p_upd->pf_update( p_subpicture, &cpy );

    video_format_Clean( &p_private->src );
    video_format_Clean( &p_private->dst );

    video_format_Copy( &p_private->src, params->p_fmt_src );
    video_format_Copy( &p_private->dst, params->p_fmt_dst );
}
