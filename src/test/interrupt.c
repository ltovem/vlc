...
int main (void)
{
    ...
    assert(!vlc_clone(&th, test_thread_simple, ctx, "test-thread-sim"));
    vlc_interrupt_raise(ctx);
    vlc_sem_post(&sem);
    vlc_sem_post(&sem);
    vlc_join(th, NULL);

    assert(!vlc_clone(&th, test_thread_cleanup, ctx, "test-thread-cle"));
    vlc_join(th, NULL);

    assert(!vlc_clone(&th, test_thread_cancel, ctx, "test-thread-can"));
    vlc_cancel(th);
    vlc_join(th, NULL);
